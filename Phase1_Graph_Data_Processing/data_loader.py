...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        # trips = trips[
        #     trips["PULocationID"].isin(bronx) & trips["DOLocationID"].isin(bronx)
        # ]
        ...
        # Convert to CSV and store in the import directory
        # save_loc = (
        #     "/var/lib/neo4j/import/" + file_path.split("/")[-1].split(".")[0] + ".csv"
        # )
        save_loc = "/var/lib/neo4j/import/" + file_path.split(".")[0] + ".csv"
        trips.to_csv(save_loc, index=False)

        # Load data into Neo4j using LOAD CSV
        with self.driver.session() as session:
            # Create nodes for pickup and drop-off locations and relationships for each trip
            session.run(
                """
    ...
    while attempt < total_attempts:
        try:
            data_loader = DataLoader(
                "neo4j://localhost:7687", "neo4j", "project1phase1"
            )
